<?php 
    ...
    $id = $_POST['id'];
    $sql = mysqli_query($con,"SELECT loginform.firstname,accessories.name,accessories.price,reservation.reservationdate,loginform.cnumber FROM loginform,reservation,accessories WHERE accessories.id = reservation.itemid AND reservation.username = loginform.username AND reservation.id = $id AND reservation.status != 'Confirmed' AND reservation.status != 'Done' AND reservation.status != 'Reject'");
