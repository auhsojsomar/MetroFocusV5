...
$sql = mysqli_query($con,"SELECT reservation.id AS id,reservation.username AS username,reservation.reservationdate AS reservedate,parts.name AS itemname,reservation.itemquantity AS itemquantity,reservation.status AS status FROM reservation,parts WHERE reservation.deleted = 0 AND reservation.category = 'Parts' AND itemid = parts.id ORDER BY reservation.id DESC");
