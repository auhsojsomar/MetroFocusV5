...
while ($row = $query->fetch_assoc()) {
    $actionButton = '<button style="width:20px;" type="button" name="confirm" id="'.$row["id"].'" class="button is-info is-small tooltip" data-tooltip="Confirm"><i class="fal fa-edit"></i></button><button style="width:20px;" type="button" name="reject" id="'.$row["id"].'" class="button is-danger is-small tooltip" data-tooltip="Reject"><i class="fal fa-trash-alt"></i></button><button style="width:20px;" type="button" name="done" id="'.$row["id"].'" class="button is-success is-small tooltip" data-tooltip="Done"><i class="fal fa-check"></i></button>';
    $output['data'][] = array(
        $row['username'],
        $row['reservedate'],
        $row['itemname'],
