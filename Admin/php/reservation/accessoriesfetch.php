...
while ($row = $query->fetch_assoc()) {
    $actionButton = '<button style="width:20px;" type="button" name="confirm2" id="'.$row["id"].'" class="button is-link is-small tooltip" data-tooltip="Confirm"><i class="fal fa-file-check"></i></button><button style="width:20px;" type="button" name="reject2" id="'.$row["id"].'" class="button is-danger is-small tooltip" data-tooltip="Reject"><i class="fal fa-times"></i></button><button style="width:20px;" type="button" name="done2" id="'.$row["id"].'" class="button is-success is-small tooltip" data-tooltip="Done"><i class="fal fa-check"></i></button>';
    $output['data'][] = array(
        $row['username'],
        $row['reservedate'],
        $row['itemname'],
