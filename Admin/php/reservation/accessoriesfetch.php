...
$sql = mysqli_query($con,"SELECT reservation.id AS id,reservation.username AS username,reservation.reservationdate AS reservedate,accessories.name AS itemname,reservation.itemquantity AS itemquantity,reservation.status AS status,reservation.reservationid AS reservationid FROM reservation,accessories WHERE reservation.deleted = 0 AND reservation.category = 'Accessories' AND itemid = accessories.id ORDER BY reservation.id DESC");
