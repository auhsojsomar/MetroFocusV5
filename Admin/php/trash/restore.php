<?php
    ...
    if($type == 'Brand'){
        ...
        if($action == 'Restore'){
            ...
            $row = mysqli_fetch_array($sql);
            mysqli_query($con,"INSERT INTO activitylogs (name,action,datemod,type,user)VALUES('$row[0]','Recover','$date','FAQ','$user')");
            mysqli_query($con,"UPDATE faq SET deleted = 0 WHERE id = $id");
        }
        else if($action == 'RestoreAll'){
            foreach($id as $newid){
                ...
                mysqli_query($con,"UPDATE faq SET deleted = 0 WHERE id = $newid");
            }
        }
    }
?>