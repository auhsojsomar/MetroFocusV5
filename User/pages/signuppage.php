				...
				<div class="columns form-container is-centered">
					<div class="column is-half">
						<form method="POST" name="signupform" id="signupform" onsubmit="return Validate();">
							...
							<div class="field">
								<label class="label">Date of Birth</label>
									<div class="control has-icons-right has-icons-left">
										<input readonly maxlength="200" class="input" type="text" id="bday" name="bday">
										<span class="icon is-small is-right">
							...
							<hr>
							<p class="note">By clicking “Create an account” below, you agree to our terms of service and privacy statement.
...
<script>
	...
	var bday_err = document.getElementById('bdaymessage');
	...
	function validall() {
		...
		if (namereg2.test(lname.value)) {
			...
			$('#lnamemessage').html('Invalid format');
			$('#lname').addClass('is-danger');
		} else if ((fname.value != "") && (lname.value != "") && (email.value != '') && (username.test(email.value)) && (
				password.value.length != 0) && (password.value.length > 7) && (cpassword.value.length != 0) && (cpassword.value ==
				password.value) && (exist != "Exist") && (!namereg.test(lname.value)) && (!namereg.test(fname.value)) && (!
				nameregdot.test(fname.value)) && (!nameregdot.test(lname.value)) && (cnumber.value.length > 10) && (cnum.test(
				cnumber.value)) && (email.value.length > 5) && (cexist != "Exist") && (bday.value != "")) {
			$.ajax({
